...
- The wasm build step is mandatory: `harmoni-wasm` is a workspace member the
  ...
  same gotcha documented in the `sandbox-gotchas` skill). `publish.yml` builds
  it once, unscoped, so `pnpm install` can resolve the workspace link
  `apps/workbench` depends on (`"harmoni-wasm": "workspace:*"`) — then
  re-scopes the same `pkg/` directory to `@primitiv-ui/harmoni-wasm` via
  `npm pkg set name=...` right before publishing it. That rename happens
  *after* `pnpm install`, so it never affects local workspace resolution;
  nothing downstream in the job re-reads it by name.
- `@primitiv-ui/harmoni-wasm` is public and MIT, same posture as the
  `crates/harmoni-*` engine it wraps (RFC 0028 §6.2) — the harmoni Figma
  plugin (`primitiv-ui/harmoni`, a private repo) consumes it as an ordinary
  published dependency, since `crates/harmoni-wasm/pkg/` is gitignored and
  generated by `wasm-pack`, so a git dependency isn't possible.
