...
| DescriptionList | ✓ | — | ✓ | ✓ | RFC 0012 D10 / RFC 0023. Registry-only prose family — no headless companion. `<dl>` compound (`DescriptionList.Term`/`.Details`); term fixed `font-weight: font-weight-semibold` across every density (D10) — CSS separates Figma's "fontStyle" instance-naming into `font-weight`, so the term binds weight, not style. No new *colour* tokens; D10 is silent on spacing, so pair-gap/term-gap/details-indent got a new density-scaled `description-list/*` Context family mirroring List's item-gap/marker-gap/indent (D9). Kitchen-sink = styled pair in the intro article |
| Divider | ✓ | ✓ | ✓ | ✓ | |
