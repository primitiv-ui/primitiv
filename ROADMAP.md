...
- [x] Combobox — all four stages landed (single-select, consumer-owned filtering, virtual-focus cursor shared with Listbox; no new design tokens). Design settled on the Figma "Combobox — exploration" page. Four things found by rendering it and then fixed are in `docs/combobox-future-work.md` §0 — the top layer, light dismiss, clearing-clears-the-value, and the docs site's component list; three headless follow-ups remain open (the cursor not scrolling into view §1.0, groups §1.1, the decorative chevron §1.2) alongside the exploration's own deferrals (built-in `filter`, async loading, multi-select, virtualization, the Command Palette composite)
- [x] Listbox
