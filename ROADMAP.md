...
| Combobox | ✓ | ✓ | ✓ | ✓ | WAI-ARIA APG Combobox pattern. Figma set `1816:61259` on page "Combobox" (10 variants, Variant[closed\|open] × Size[xs-xl], md-first); design fully settled 2026-08-13 on the "Combobox — exploration" page (`1816:60308`, all seven calls), deferred items in `docs/combobox-future-work.md`. **Needs no new design tokens of any kind** — that falls straight out of two decisions: the control is **Input verbatim** (§B1 — it *is* an editable text field, so `framed-control/{size}/*` and Input's own states), and the popup is a **Dropdown panel** at `elevation/overlay` (§A1). Rows are `Listbox / Option`, not `Dropdown / CheckboxItem`, because Listbox's row is the only one in the library with a **cursor** state — which is what virtual focus needs, and why the two components share their whole row language (`dropdown/{size}/item/*`, cursor tint on `[data-highlighted]`, hover deliberately weaker at `action/ghost/hover`, checkmark always in flow reserving its gutter via `visibility`). Headless = `Root`/`Input`/`Content`/`Item`/`Empty`; filtering is **consumer-owned** (no `filter` prop), the input holds the live query while open and resets from the committed value on Enter or Escape, and `Content` **unmounts** while closed. Registry `combobox` is hand-authored for one forced reason: **you cannot put a chevron inside an `<input>`**, so Figma's single Input frame becomes a `__control` wrapper around a bare field, and every state (focus ring, invalid, disabled, chevron flip) is read off the inner input with `:has()`. **`size` goes on the root, once** — unlike Select, whose root renders no element in rich mode and so repeats the axis on trigger *and* content; here the headless root is a real `<div>` containing both halves, so the knobs are declared there and inherit down the DOM (which the fixed-position panel does too — custom properties inherit through the DOM, not the containing block). That also matches the Figma set's single Size axis. Anchor positioning wires itself from `useId()` (the dropdown / breadcrumb-overflow fix), so two comboboxes on a page cannot collide. **`position: fixed` alone was observed to fail, and the panel carries `z-index: 1000`**: the first build used fixed + anchor positioning with `Portal` documented as the stacking escape hatch, and rendering it showed the kitchen-sink's own disabled-Combobox demo painting over the open panel (`opacity: 0.5` forms a stacking context later in the DOM). The z-index is deterministic *because* none of the panel's ancestors forms a stacking context, so its nearest one is the root element — confirmed fixed in a browser. A middle attempt (`popover="manual"` + `showPopover()` on mount) did **not** fix it and was reverted, its cause unknowable from the sandbox since **jsdom's `showPopover()` is a no-op stub**. §G had called for the Popover API and it is still wanted — in the **headless** layer, where `popover="auto"` would also supply the missing outside-click dismiss and make an exit animation possible. Four headless follow-ups recorded in `docs/combobox-future-work.md`: **no outside-click dismiss** (§1.0, the most user-visible), **Escape over a cleared query restores the old label** (§1.0b — exactly what exploration §D1 specified, but §D1 never considered that clearing the field is itself a deselect intent; needs a human call), the top layer (§1.2), and no groups (`ComboboxGroup`/`GroupLabel` need the headless to own `role="group"` + `aria-labelledby`, as Listbox's already do — a registry-only half-version would leave ids to the consumer). Plus the chevron is decorative `ComboboxIcon` rather than an interactive `ComboboxTrigger` (Select's whole frame is a `<button>` so its chevron opens for free; a `<div>` wrapping an `<input>` has no such luck, and the headless exposes no open-toggle). Kitchen-sink = 3 demos after Listbox on purpose (its last demo wires an Input to a Listbox by hand; Combobox *is* that composition done properly): single-select with string children, the §B1 search flavour (leading magnifier, no chevron, full `[mark][leading][label][trailing]` row anatomy), and invalid + disabled |
...
- [x] Combobox — all four stages landed (single-select, consumer-owned filtering, virtual-focus cursor shared with Listbox; no new design tokens). Design settled on the Figma "Combobox — exploration" page; the exploration's own deferrals (built-in `filter`, async loading, multi-select, virtualization, the Command Palette composite) **and three headless follow-ups found while building the registry** — groups, the Popover-API popup layer, and an interactive chevron — are all recorded in `docs/combobox-future-work.md`
- [x] Listbox
