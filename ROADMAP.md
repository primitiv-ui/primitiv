...
| Select | ✓ built (rich + native) | ✓ | Workspace picker and Collection dropdown; the rich Popover-API listbox is the default (`native` opts back into the `<select>` wrapper). The separately-deferred Combobox is now built headless — see [`src/Combobox/README.md`](packages/react/src/Combobox/README.md) |
...
NavigationMenu order). One row below is a genuine backlog gap, not a
deliberate omission: **InputGroup** already has a registry + kitchen-sink
surface and is missing only a Figma set.
**MillerColumns** and **Tree** were both built headless ahead of a concrete
need — RFC 0013 proposed them as candidates for a Harmoni-plugin "destination
browser" and never settled which to ship — but both are now complete across
all four stages (Tree 2026-08-07, MillerColumns 2026-08-08) and the RFC 0013
question is decoupled from their readiness rather than blocking it.
...
| Combobox | ✓ | ✓ | ✓ | ✓ | WAI-ARIA APG Combobox pattern. Figma set `1816:61259` on page "Combobox" (10 variants, Variant[closed\|open] × Size[xs-xl], md-first); design fully settled 2026-08-13 on the "Combobox — exploration" page (`1816:60308`, all seven calls), deferred items in `docs/combobox-future-work.md`. **Needs no new design tokens of any kind** — that falls straight out of two decisions: the control is **Input verbatim** (§B1 — it *is* an editable text field, so `framed-control/{size}/*` and Input's own states), and the popup is a **Dropdown panel** at `elevation/overlay` (§A1). Rows are `Listbox / Option`, not `Dropdown / CheckboxItem`, because Listbox's row is the only one in the library with a **cursor** state — which is what virtual focus needs, and why the two components share their whole row language (`dropdown/{size}/item/*`, cursor tint on `[data-highlighted]`, hover deliberately weaker at `action/ghost/hover`, checkmark always in flow reserving its gutter via `visibility`). Headless = `Root`/`Input`/`Content`/`Item`/`Empty`; filtering is **consumer-owned** (no `filter` prop), the input holds the live query while open and resets from the committed value on Enter, Escape **or a click outside**, and `Content` **unmounts** while closed. `Content` renders `popover="auto"` and promotes itself with `showPopover()` (the `useSelectContent` pattern), which supplies both the top layer and light dismiss — the browser hit-tests and reports back via a `toggle` event, so there is no hand-rolled outside-click listener; a dismiss runs the full `dismiss()`, not just a close, so all three exits land in the same state. **Emptying the field clears the value** (`onValueChange("")`), guarded so an already-empty field fires nothing — exploration §D1 had only weighed "restore the value" against "keep the raw query" and never considered that clearing is itself a deselect intent. Registry `combobox` is hand-authored for one forced reason: **you cannot put a chevron inside an `<input>`**, so Figma's single Input frame becomes a `__control` wrapper around a bare field, and every state (focus ring, invalid, disabled, chevron flip) is read off the inner input with `:has()`. **`size` goes on the root, once** — unlike Select, whose root renders no element in rich mode and so repeats the axis on trigger *and* content; here the headless root is a real `<div>` containing both halves, so the knobs are declared there and inherit down the DOM (which the fixed-position panel does too — custom properties inherit through the DOM, not the containing block). That also matches the Figma set's single Size axis. Anchor positioning wires itself from `useId()` (the dropdown / breadcrumb-overflow fix), so two comboboxes on a page cannot collide. **Getting the panel into the top layer took three attempts, all recorded in `docs/combobox-future-work.md` §0.1** — worth reading before touching the panel, because every failure was a real-browser paint bug no test here would catch. (1) `position: fixed` + anchor positioning alone: escapes overflow clipping but still competes in the page's stacking contexts, and the kitchen-sink's own disabled-Combobox demo painted over the open panel (`opacity: 0.5` forms a stacking context, later in the DOM). (2) `popover="manual"` + `showPopover()` in the registry wrapper: did nothing, because `manual` popovers have **no light dismiss** and there was no `toggle` listener — reverted. (3) `popover="auto"` + `toggle` listener in the **headless**: correct, and the established `useSelectContent` pattern. A `z-index: 1000` shipped briefly in between and did fix the overlap; removed once the top layer landed, since a top-layer element ignores z-index and a knob that does nothing is worse than none. Two traps left behind: the `[popover]` UA reset (`margin: auto` + `inset: 0` centre it and fight the anchor insets) is **not optional** — deleting it visibly misaligned the popup — and `display` must be set only under `:popover-open`, an ungated `display` having been tried as a "fail open" hedge whose fallback state was itself the broken one. **The cursor scrolls into view** via a `moveCursor` funnel copied from Listbox's own (`setActiveValue` + `scrollIntoView({ block: "nearest" })` in one callback, so no path can move the cursor without bringing it on screen) — found because `ComboboxItemMeta.element` had no reader at all, making its "kept for future scroll-into-view work" note an unfinished feature rather than a spare field, with a live symptom once the 18rem panel scrolls. Listbox was checked and does **not** share the gap. Two headless follow-ups remain: §1.1 no groups (`ComboboxGroup`/`GroupLabel` need the headless to own `role="group"` + `aria-labelledby`, as Listbox's already do; a registry-only half-version would leave ids to the consumer) and §1.2 the decorative chevron (`ComboboxIcon`, not an interactive `ComboboxTrigger`: Select's whole frame is a `<button>` so its chevron opens for free, but a `<div>` wrapping an `<input>` has no such luck and the headless exposes no open-toggle). Kitchen-sink = 3 demos after Listbox on purpose (its last demo wires an Input to a Listbox by hand; Combobox *is* that composition done properly): single-select with string children, the §B1 search flavour (leading magnifier, no chevron, full `[mark][leading][label][trailing]` row anatomy), and invalid + disabled |
...
| Container | ✓ | registry only | ✓ | ✓ | RFC 0022 (layout primitives, build-order step 3) — unblocked by RFC 0025's breakpoint scale. Registry-only — a centred, max-width content column. `size` (xs–2xl\|full, default lg) resolves **straight against the breakpoint primitives** (`var(--primitiv-breakpoint-lg)`) rather than a container-specific width scale: one source of truth, and a Context family would be the wrong collection since Context is density-scoped and a max-width shouldn't tighten with density. `gutter` (responsive\|none\|sm\|md\|lg) is a new density-scaled `container/gutter-{sm,md,lg}` Context family; `responsive` (the default) steps up at md and lg. `box-sizing: border-box`, so the gutter sits inside the cap (Tailwind/Chakra behaviour). Full-bleed is deliberately two knobs — `size="full"` drops the cap, `gutter="none"` drops the padding. No `centerContent` prop: Container centres itself, `Center` centres content within it. Taking the `container/*` Context namespace required moving the pre-existing size-slotted padding/gap/radius family to `surface/*` (adopted by no registry component; only the workbench's OklchPicker/PluginFrame read its radius). Figma set landed 2026-08-09 on page "Container" (`1765:41081`, 7 variants, one `Size` axis, **lg-first** so the dropdown defaults correctly — `defaultVariant` is read-only via the plugin API): each frame's width **binds to `breakpoint/*`** rather than a literal, so a `Container` at `Size=lg` and a design frame at `lg (1024)` are the same width by construction. One recorded divergence — Figma auto-layout padding takes a single value, so the set binds `container/gutter/md` where CSS escalates sm → md at 48rem → lg at 64rem. Deliberately **no `Grid` set**: a grid's value is its reflow, which a fixed-width frame cannot express, so it joins Box/Stack/Spacer/Center/AspectRatio as `—`. Kitchen-sink = three containers in one parent showing cap and gutter |
...
| List | ✓ | registry only | ✓ | ✓ | RFC 0012 D9 / RFC 0023. Registry-only prose family — no headless companion. Custom `::before` markers (bullet/counter) instead of native `::marker`, so both marker colour (`list/marker/foreground`) and marker↔text gap (`list/marker-gap`) are controllable — the native pseudo-element has no controllable gap. `type` unordered\|ordered, `indent` toggle, `size` scales type only (item-gap/marker-gap/indent are density-scaled, not size-scaled). **`marker` toggle added 2026-08-14** — the `list-style: none` affordance for prose/landing-page lists. CSS is `content: none` on the `::before`, which generates **no box**, so the marker's flex `gap` goes with it and the row sits genuinely flush (`display: none`/`visibility: hidden` would each leave a marker-width column behind); the rule must stay **after** the `--ordered` counter rule, since the two have identical specificity and source order is all that separates them — placed first, an ordered list silently keeps its numbers. Independent of `indent`; both `false` = fully flush. Figma: `ListItem` gained a `Show marker` BOOLEAN (still 20 variants) and the composed `List` a `Marker` true\|false **variant axis** (20 → 40) — a variant rather than a boolean because the markers live inside nested `ListItem` instances and `componentPropertyReferences` on an instance sublayer throws, so a parent boolean cannot reach them. Confirmed by measurement (md unordered reflows 136px → 120px) and, once again, `clone()` dropped all four `Show Item 5-8` refs on every one of the 20 clones — repaired and re-read (**fourth occurrence** of clone-drops-refs). Kitchen-sink = both types, a disabled row, and a flush marker-less list in the intro article |
| Listbox | ✓ | ✓ | — | — | WAI-ARIA APG Listbox pattern. Figma sets landed 2026-08-06 (page `1569:150`: `Listbox / Option` 40, `CheckboxOption` 40, `Empty` 5, composed `Listbox` 15 — all md-first, the composed set exposing a real Slot whose `preferredValues` curate the row components); design fully settled on the "Listbox — exploration" page, see `docs/listbox-future-work.md`. Registry surface and kitchen-sink demo still to build. Uses a **virtual-focus** cursor (`aria-activedescendant` on the root, `data-highlighted` on the option) instead of a roving tabstop, which is the whole reason it is a separate component from `Select`'s popup listbox: DOM focus never leaves the root, so an external input can hold focus and drive the list. **All three APG examples are covered**: Scrollable (via `selectionFollowsFocus`), Grouped Options (via `Listbox.GroupLabel`, which names the group with `aria-labelledby` at a visible heading), and Rearrangeable — the latter drove the multi-select modifier shortcuts (Shift+Arrow, Ctrl+Shift+Home/End, Ctrl+A), chord pass-through so `Alt+Arrow` reaches a consumer toolbar, and DOM-order navigation that survives in-place reordering (registration order goes stale because React keeps the same elements mounted). Disabled options are a house extension — APG does not specify them. Unblocks the Tier 3 Command Palette and Search-with-suggestions composites |
| MillerColumns | ✓ | ✓ | ✓ | ✓ | Finder-style column view. Figma landed 2026-08-08 on page "Miller Columns", five md-first sets: `Miller Columns / Item` (30, Size × State rest\|hover\|ancestor\|terminal\|focused\|disabled), `/ Resize Handle` (3, State only — the 6px grip is flat across sizes because a pointer target is ergonomics, not typography), `/ Column` (10, with real `Rows`/`Empty` slots), `/ Preview Panel` (10, `Content`/`Empty` slots) and the composed `Miller Columns` (5), which nests real Column and Preview instances so a change to Item propagates. Every attribute binds to a variable — audited to zero literals across all 58 variants. Decisions came from the eleven-section "Miller Columns — exploration" page (`1738:29132`): a **continuous** surface cut by one hairline per seam rather than separated panels (the resize grip IS the seam, and a gap gives it no line to sit on); **graded neutral selection depth** — ancestors at `tree/row/hover`, the terminal row at `action/ghost/active` — because the headless marks every path item selected and, styled alike, there is no way to tell which row was clicked (the obvious one-step gap was drawn, and is too weak to read); a **trailing chevron that never rotates** (it encodes "children open to the right", a direction, which is why a leaf reserves no slot and mixed columns stay aligned); and full-bleed **square** bands whose only rounded element is the focus ring. That ring is **inset**, unlike Button's and Tree's: a column is `overflow-y: auto` and per CSS Overflow 3 the other axis then computes to `auto`, so an outset ring is always clipped — and the fill ramp is already spent on selection depth, so `select`'s fill-instead-of-ring precedent has nowhere to go. The exploration produced **headless work**, not just styling: `data-terminal` on the deepest selected item (the hook decision D needed), `data-empty` on Column and PreviewPanel, a keyboard-operable `ResizeHandle` (`minWidth`/`maxWidth`/`step`, the `aria-value*` trio backed by a ResizeObserver, arrow + Home/End stepping), an **rtl fix** (the horizontal arrow pair was hardcoded, so the step-in key pointed away from the child column), typeahead scoped to the focused column, and the `role="tree"` moved off the strip onto an inner widget so `PreviewPanel` is no longer an illegal child of a tree. 100% lines/branches/functions/statements, 99 tests. The 55-token `miller-columns/*` Context family aliases the size/space/radii ladders throughout, so — unlike `tree` — it needs **no** `LENGTH_CATEGORIES` entry. Registry landed 2026-08-08 (roster count 57, no component dependencies — the chevron ships inline); hand-authored like NavigationMenu, so no committed-wrapper drift test. The resize seam eases its **width and colour** in both directions and the column border yields via `:has()` while the grip is live, so the seam is never drawn twice. Kitchen-sink: a Finder-style file browser after Listbox, including a childless branch so the empty-column state is exercised |
...
| Tree | ✓ | ✓ | ✓ | ✓ | Figma landed 2026-08-07 on page "Tree" (`1583:3`), five md-first sets: `Tree / Item` (`1590:3`, 25), `Tree / Branch Control` (`1611:13`, 50), `Tree / Connector` (`1674:64`, 30), `Tree / Selection Path` (`1733:1215`, 10) and the composed `Tree` specimen (`1733:1790`, 5). Decisions came from the nine-section "Tree — exploration" page (`1567:43`): a dedicated, tighter `tree/{size}/item/*` family rather than a `dropdown/*` alias (tree lists run long); connector lines **on by default** with an off toggle; row labels on `body/*` **one tier down** from the row's own Size (xs\|sm→body/xs · md→body/sm · lg→body/md · xl→body/lg), unlike Dropdown's 1:1. Connectors are a separate set, not a per-row variant, because depth is unbounded — a row can need a rail from every unclosed ancestor at once, which no finite variant axis can enumerate; `Style=passthrough`/`rail` maps to the `border-inline-start` rail on a `data-depth` group, `Style=elbow` to its `:last-child` termination, and `Target=branch\|leaf` to the existing `data-branch`/`data-leaf` attributes — so the whole connector feature stays pure registry CSS with **zero new headless attributes**. `Target` exists because a branch row's stub must stop before its chevron while a leaf's slot is empty, leaving a branch-length stub ~16/17/21/26/30px short at xs–xl. `Tree / Selection Path` composes `Breadcrumb/Item` + `Breadcrumb/Separator` directly, exactly as headless `Tree.SelectionPath` does — **the registry will reuse `breadcrumb` rather than grow its own path anatomy** (settled 2026-08-07): a `tree` component declares `dependsOn: ["breadcrumb"]` and adds only the `[data-empty]` em-dash and the multi-select stacked-trails layout. The 55-token `tree/*` Context family is mirrored into `packages/tokens/src/context.json`; the two literal `connector/stub-width*` families are why `tree` is the one component namespace in the emitter's `LENGTH_CATEGORIES`. Registry `tree` landed 2026-08-07 (roster count 56, `dependsOn: ["breadcrumb"]`). Hand-tuned like Accordion/Collapsible, so no committed-wrapper drift test: **`TreeBranchContent` force-mounts and adds the `__branch-content-inner` clip + `__branch-content-body`** to drive the same `display: grid` 0fr↔1fr row-track open/close transition Accordion uses (branches animate to their subtree's exact height, no `max-height` guess, disabled under `prefers-reduced-motion`); `TreeItem` renders a leading `__item-spacer` so leaf and branch labels align; and **`TreeSelectionPath` uses the headless render prop to rebuild each trail from the REGISTRY Breadcrumb parts**, which is what "reuse breadcrumb" means concretely — no second path anatomy, and `size` maps one tier down (md→Breadcrumb sm). **Indentation comes from nesting, not `data-depth`**: `BranchContent` physically contains its children, so one `padding-inline-start` on the body indents a whole subtree at any depth — sidestepping the headless README's `attr(data-depth type(<integer>))` suggestion, which Firefox does not support. Connectors are pure CSS with zero new headless hooks: a `::after` on each nested treeitem spans its full height (so an ancestor's rail continues past deeper rows), `:last-child` truncates it into the elbow, and a `::before` draws the stub — longer for a leaf than a branch, the Figma `Target` axis. Selected/hover/focus are drawn on the *row*, never the treeitem, because the roving tabstop sits on `TreeBranch`, which wraps the whole subtree. Kitchen-sink: a three-column "Tree" showcase after Tooltip (connectors + selection path, connectors off/text only, multiple selection at sm). The RFC 0013 destination-browser question (Tree vs MillerColumns) is unchanged |
...
- [x] Combobox — all four stages landed (single-select, consumer-owned filtering, virtual-focus cursor shared with Listbox; no new design tokens). Design settled on the Figma "Combobox — exploration" page. Five things found by rendering it and then fixed are in `docs/combobox-future-work.md` §0 — the top layer, light dismiss, clearing-clears-the-value, cursor scroll-into-view, and the docs site's component list; two headless follow-ups remain open (groups §1.1, the decorative chevron §1.2) alongside the exploration's own deferrals (built-in `filter`, async loading, multi-select, virtualization, the Command Palette composite)
- [x] Listbox
