/**
 * arrange-breadcrumb-item-component-set.js
 *
 * Positions all variants in the Breadcrumb/Item component set into the
 * documented grid. Run this after building or restructuring variants.
 *
 * Grid structure:
 *   Rows    → Size (md first, then xs sm lg xl)
 *   Columns → State (link / current)
 *
 * Breadcrumb/Item is a single text label, not a framed control. It has no
 * Interaction sub-axis — State is the only column dimension, the same shape
 * as Field's grid. Density is controlled by the containing frame's Context
 ...
 *   State = link | current
 *
 * State was added 2026-07-27 so the current-page item reads in the primary
 * foreground (content/primary) while ancestor links read in the muted
 * foreground (content/muted) — previously both rendered identically on
 * content/secondary.
 *
 * ─── Usage ───────────────────────────────────────────────────────────────────
 *  1. Select the Breadcrumb/Item component set on the canvas.
...
(async function () {
  ...
  const STATE_ORDER = ["link", "current"];
  ...
  if (!componentSet) {
    console.error("Nothing selected. Select the Breadcrumb/Item component set and re-run.");
    return;
  }
  ...
  // Labels
  const existing = figma.currentPage.findOne(n => n.name === "Breadcrumb/Item Grid Labels");
  ...
  labelGroup.name  = "Breadcrumb/Item Grid Labels";
