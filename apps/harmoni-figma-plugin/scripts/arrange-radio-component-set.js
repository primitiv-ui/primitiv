// arrange-radio-component-set.js
// Arranges the Radio component set (40 variants) into a labelled grid.
// Run via figma_execute by replacing the selection lookup with a direct getNodeByIdAsync call.
//
// Grid axes:
//   Rows: Size (md first, then xs sm lg xl)
//   Cols: State (unchecked | checked) × Interaction (default | hover | focus | disabled)
//
// Density is controlled by the containing frame's Context variable mode override.
// Re-run safe: deletes "Radio Grid Labels" group before regenerating.
...
(async function () {
  ...
  // Replace with direct ID lookup when running via figma_execute:
  // const set = await figma.getNodeByIdAsync('401:17958');
  const set = figma.currentPage.selection.find(n => n.type === 'COMPONENT_SET');
  if (!set) {
    console.error('Nothing selected. Select the Radio component set and re-run.');
    return;
  }
  ...
  // Labels
  const existing = figma.currentPage.findOne(n => n.name === 'Radio Grid Labels');
  ...
  for (const st of STATE_ORDER) {
    ...
    const header = makeLabel(st.charAt(0).toUpperCase() + st.slice(1), 0, set.y - 48, true);
    header.x = set.x + (firstX + groupRight) / 2 - header.width / 2;
  ...
  labelGroup.name  = 'Radio Grid Labels';
