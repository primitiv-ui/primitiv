// Arrange Tabs/Trigger component set
// Grid: 20 rows (Size md-first × 4 Positions) × 8 cols (State active|inactive × 4 Interactions)
...
const TRIGGER_SET_ID = '425:5528';
const EDGE    = 24;
const CGAP    = 16;  // gap between interaction columns within a state group
const GRPGAP  = 32;  // extra gap between active and inactive column groups
...
const POSITIONS = ['standalone', 'start', 'middle', 'end'];
...
const set = await figma.getNodeByIdAsync(TRIGGER_SET_ID);
...
for (const sz of SIZES) {
  const s = variants[`${sz}/standalone/active/default`];
  rowH[sz] = s ? s.height : 32;
}

// Column definitions: active × 4 interactions, then inactive × 4 interactions
const cols = [];
...
for (let si = 0; si < SIZES.length; si++) {
  ...
  for (const pos of POSITIONS) {
    ...
    for (let c = 0; c < cols.length; c++) {
      ...
      if (c === 3) curX += GRPGAP; // gap between active and inactive groups
...
const old = page.findOne(n => n.name === 'Tabs/Trigger Grid Labels');
if (old) old.remove();
...
if (lblNodes.length) {
  ...
  grp.name = 'Tabs/Trigger Grid Labels';
}

// Default instance: md/standalone/active/default at index 0
const def = variants['md/standalone/active/default'];
