 ...
 *
 * Appends two screens to the "Wireframes — Harmoni Plugin" page
 * (creates the page if it doesn't exist yet):
 *
 *   Screen 5 — Swatch style: Square shape active, A11y badges on
 *              Palette editor showing the Swatch Style section with Square
 *              selected in the toggle and A11y badges enabled; the left
 *              column preview updates to show square-cornered swatches.
 *
 *   Screen 6 — Canvas output preview
 *              Simulates the Figma canvas (dark background) with three
 *              generated swatch frame variants placed on it.
 *
 * HOW TO RUN
 * ----------
...
(async function createSwatchConfigWireframes() {
  ...
  function mkSubFrame(p,name,x,y,w,h,bg) {
    const f=figma.createFrame(); f.name=name; f.x=x; f.y=y;
    f.resize(w,h); f.fills=solid(bg||"#FFFFFF"); f.clipsContent=false;
  ...
  function mkSec(p,s,x,y,col) { return mkText(p,s,x,y,10,"Medium",col||"#999999"); }
  ...
  // ─── Canvas output frame helper (Screen 6 only) ───────────────────────────
  // Swatch output sub-frame: tight fit around the ramp (16px inset each side).
  const SF_W   = RAMP_W + 32;                        // 388px
  const SF_X   = Math.round((W - SF_W) / 2);         // centred in 640px frame
  const SF_RAMP_X = 16;                              // ramp starts 16px into sub-frame

  function drawSwatchOutputFrame(p, name, yPos, mode) {
    const BSH = mode==="rd-a11y" ? SW : mode==="sq-a11y" ? 60 : 40;
    let sfH = Math.ceil((8 + BSH + 4 + 10 + 8) / 4) * 4;
    const sf = mkSubFrame(p, name, SF_X, yPos, SF_W, sfH, "#FFFFFF");
    sf.cornerRadius=6;
    for (let i=0;i<10;i++) {
      const l=LIGHTNESS[i], sx=SF_RAMP_X+i*(SW+SGAP), fg=ink(l);
      if (mode==="sq-a11y") {
        mkRect(sf,`Swatch ${STEPS[i]}`,sx,8,SW,BSH,grey(l)).cornerRadius=2;
        mkText(sf,FG[i],sx+2,16,7,"Bold",fg); mkText(sf,RATIOS[i],sx+2,27,7,"Regular",fg); mkText(sf,"AA",sx+2,38,7,"Bold",fg);
      } else if (mode==="rd-a11y") {
        mkRect(sf,`Swatch ${STEPS[i]}`,sx,8,SW,BSH,grey(l)).cornerRadius=SW/2;
        mkText(sf,"AA",sx+6,16,7,"Bold",fg);
      } else {
        mkRect(sf,`Swatch ${STEPS[i]}`,sx,8,SW,BSH,grey(l)).cornerRadius=2;
      }
      mkText(sf,STEPS[i],sx+1,8+BSH+4,7,"Regular","#555555");
    }
    return sfH;
  }
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // SCREEN 5 — Swatch style: Square shape active, A11y badges on
  // Left column shows square-cornered swatches to reflect the live preview.
  // ═══════════════════════════════════════════════════════════════════════════
  const s5=mkPaletteEditor(
    "Screen 5 — Swatch style (Square + A11y)", PITCH*4, "Acme Corp", "Neutral",
    { shapeActive:"Square" },
    { shapeActive:"Square", a11yBadges:true }
  );

  // ═══════════════════════════════════════════════════════════════════════════
  // SCREEN 6 — Canvas output preview (dark Figma canvas)
  // ═══════════════════════════════════════════════════════════════════════════
  const s6=mkFrame("Screen 6 — Canvas output preview",PITCH*5,0,W,H,"#3C3C3C");
  mkRect(s6,"Figma toolbar",0,0,W,HDR_H,"#2C2C2C");
  mkText(s6,"Harmoni",PAD,15,14,"Medium","#FFFFFF");
  mkRect(s6,"Plugin handle",W-50,14,34,20,solidA("#FFFFFF",0.1)).cornerRadius=4;

  let y6=64;
  mkSec(s6,"SQUARE — WITH ACCESSIBILITY INFO",PAD,y6,"#888888"); y6+=16;
  y6+=drawSwatchOutputFrame(s6,"Neutral / Acme Corp",y6,"sq-a11y")+16;
  mkSec(s6,"ROUND — WITH ACCESSIBILITY INFO",PAD,y6,"#888888"); y6+=16;
  y6+=drawSwatchOutputFrame(s6,"Neutral / Acme Corp (Round)",y6,"rd-a11y")+16;
  mkSec(s6,"SQUARE — STEP NUMBERS ONLY",PAD,y6,"#888888"); y6+=16;
  drawSwatchOutputFrame(s6,"Neutral / Acme Corp (no a11y)",y6,"sq-labels");

  figma.viewport.scrollAndZoomIntoView([s5,s6]);
  console.log('✓ Added Screens 5 & 6 (Swatch style + canvas output) to "Wireframes — Harmoni Plugin".');
