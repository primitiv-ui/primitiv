...
> It generates every colour in the system. Give it one colour and it
> builds a full scale around it, choosing the text colour for each step
> and checking the contrast as it goes. It is also a Figma plugin you can
> buy separately, but you do not need it: Primitiv ships the palettes it
> produced.
...
> generates it. A larger set of standard ramps is available too, for
> colour the semantic roles do not own — see below.
...
**Second block — the standard ramps** — `heading/h3` plus `body/md`:

> **The standard ramps**

> The six palette scales exist to be pointed at. Every one of them has a
> job in the Intent tier — a surface, a border, a status — and that is
> why there are six rather than twenty.

> Plenty of colour has no job like that. A chart with nine series, a set
> of project tags, an illustration: none of it is a surface or a status,
> and none of it should be your brand colour stretched to cover a
> shortage. For that there is a standard palette — a wider set of fixed
> ramps, generated the same way and holding the same contrast, that no
> semantic role points at.

> It is off by default. Switch it on in `primitiv.json`:

```json
{ "palette": { "standard": true } }
```

> Two things follow from it being fixed. Regenerating your theme does not
> change it, so a chart keeps its colours when your brand colour moves.
> And nothing in the components uses it, so leaving it off costs you
> nothing.

> **PENDING — this block describes work that is planned, not shipped.**
> Do not publish it until the standard ramps are in `packages/tokens` and
> the `primitiv.json` key exists. Decisions behind it are recorded in
> note 7 below. The config key spelled above is a placeholder and must be
> checked against `config::resolve` before this page goes live.
...
7. **A standard palette is planned, and three decisions about it fix the
   wording on this page** (settled 2026-09-06, not yet built):
   - **It sits alongside the six semantic scales, not instead of them.**
     The six keep their jobs in the Intent tier and keep being
     regenerated by `primitiv theme --brand`; the standard ramps are
     additional and fixed. This is why Page 2 §2 still says "six scales"
     and only adds a pointer, rather than restating the tier.
   - **Harmoni generates them too, from seeds Primitiv picks.** This is
     the decision that keeps Page 5 §2 intact: "every step of every
     generated scale has a text colour that clears its minimum" stays
     true without qualification, and the ramp-quality gates in
     `crates/harmoni-core/tests/ramp_regression.rs` extend to cover them.
     Had they been hand-authored or imported, that sentence would have
     had to name which scales it covers — a much worse claim on an
     accessibility page.
   - **They are opt-in.** A Tailwind-sized set is roughly twenty ramps
     across two themes, which would dwarf the token layer for everyone
     who never uses it. That is the one cost: Page 2 §5's "all of it
     emits together" is now true of everything *except* this, which is
     why the new block spells the switch out rather than leaving it
     implicit.
   The name "standard ramps" is provisional. It reads well against the
   six *semantic* ones, but nothing has been committed to.
