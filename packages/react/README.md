...
| [Checkbox](src/Checkbox/README.md)       | A real, visually-hidden native `<input type="checkbox">` in a `<label>` box, so it participates in forms; the platform tri-state (`indeterminate` → `aria-checked="mixed"`, `:indeterminate`), controlled/uncontrolled state, `Indicator` mark, and `asChild` on the indicator                                                                                   |
...
| [Drawer](src/Drawer/README.md)           | Edge-anchored dialog — a thin composition over Modal that reuses its native `<dialog>` machinery (focus trap, inert background, top layer, Esc, bounding-rect click-outside, controlled/uncontrolled state, imperative API, `asChild`, `forceMount`), adding a `side` (`top`/`right`/`bottom`/`left`) axis emitted as `data-side` for the styled layer to slide the panel from that edge |
...
| [Listbox](src/Listbox/README.md)         | WAI-ARIA APG Listbox pattern — a persistently-visible option list using a **virtual-focus** cursor (`aria-activedescendant` on the root, `data-highlighted` on the option) rather than a roving tabstop, so an external input can hold DOM focus and drive the list; single/multiple selection with APG's Shift+Arrow / Ctrl+Shift+Home-End / Ctrl+A range shortcuts, opt-in `selectionFollowsFocus`, prefix typeahead, groups named by prop or visible `GroupLabel`, disabled options, DOM-order navigation that survives in-place reordering, orientation/RTL, and `asChild` composition on every part |
...
| [NavigationMenu](src/NavigationMenu/README.md) | ARIA APG Disclosure Navigation Menu pattern — the desktop dropdown site nav: `<nav>` landmark + `<ul>` of entries, one panel open at a time (`""` means closed), hover-intent open with instant switching and a nav-wide close grace, axis/RTL-aware arrow travel that keeps every entry in the tab order (not a roving tabindex), cross-axis arrow to enter a panel, Escape-to-close with focus return, an optional `Viewport` panels portal into so the open one morphs into the next, an optional `Indicator` publishing the open trigger's measured geometry as custom properties, and a `Link` carrying `aria-current` that the composed mobile nav (`Drawer` + `Collapsible`) reuses |
| [Popover](src/Popover/README.md)         | WAI-ARIA non-modal dialog — native HTML Popover API (`popover="auto"`) compound with trigger/anchor/content/close/title/description, controlled/uncontrolled state, light-dismiss sync, focus-in-on-open + Escape focus-return (no focus trap), CSS-anchor-positioning placement, and `asChild` composition |
...
| [SegmentedControl](src/SegmentedControl/README.md) | Single-select value picker (WAI-ARIA Radio Group semantics) styled as a linear pill strip — native `<button role="radio">` segments with controlled/uncontrolled state, roving tabindex, arrow-key navigation, horizontal/vertical orientation, per-item **and** whole-control `disabled`, `data-state` / `data-orientation` / `data-disabled` styling hooks, and `asChild` composition (use `ToggleGroup` for on/off command toggles) |
| [Select](src/Select/README.md)           | Headless wrapper over native `<select>` / `<option>` / `<optgroup>` — controlled/uncontrolled state via `onValueChange`, `Select.Placeholder` with auto-`defaultValue` inference, `Select.Group` for optgroup labelling, `data-disabled` styling hook, `asChild` composition, and Field-aware (opts into `FieldContext` for auto-wired `id` / `aria-describedby` / `aria-invalid` / `disabled` / `required`) |
...
| [SplitButton](src/SplitButton/README.md) | One primary action plus a menu of related alternatives, bound into a single `role="group"` widget — a composition over `Dropdown` that adds the coordination Dropdown alone can't provide: the group boundary, the menu trigger's accessible name derived from the primary action (so a chevron-only control is never unlabelled), `disabled` propagating from the group to both halves, and `ArrowDown` on the action opening the menu; both halves stay independently tabbable, and `asChild` composition is available on the group and the action |
...
| [Switch](src/Switch/README.md)           | A real, visually-hidden native `<input type="checkbox" role="switch">` in a `<label>` track, so it participates in forms; binary on/off state, always-mounted `Thumb` for CSS-driven sliding (off the input's native `:checked`), controlled/uncontrolled state, `data-disabled` styling hook, and `asChild` on the thumb                                              |
| [Table](src/Table/README.md)             | WAI-ARIA Table pattern — compound component wrapping standard HTML table elements with accessible headers, captions, a responsive scroll wrapper, and an opt-in expandable-row disclosure                                                                                                |
...
| [Textarea](src/Textarea/README.md)       | Headless multi-line text input — native `<textarea>` with full attribute pass-through, ref forwarding, `disabled` with `data-disabled` styling hook, `asChild` composition for wrapping autosizing implementations, and Field-aware (opts into `FieldContext` for auto-wired `id` / `aria-describedby` / `aria-invalid` / `disabled` / `required`) |
