...
describe('Tabs change event callbacks', () => {
  describe('optional onChange callback', () => {
    ...
    it('does not re-fire onChange when re-clicking a tab activated after mount', async () => {
      // No defaultValue: the guard reads an activeValue that changes after the
      // first click, so a stale activateTab closure would re-fire on the second.
      const user = userEvent.setup();
      const handleChange = vi.fn();

      render(
        <Tabs.Root onChange={handleChange}>
          <Tabs.List label="Test tabs">
            <Tabs.Trigger value="tab1">Tab 1</Tabs.Trigger>
            <Tabs.Trigger value="tab2">Tab 2</Tabs.Trigger>
          </Tabs.List>
