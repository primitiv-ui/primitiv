 ...
 *
 * Run via `pnpm --filter @primitiv-ui/tokens sync:serve` (or the root
...
server.listen(port, '127.0.0.1', () => {
  ...
  console.log(
    `[@primitiv-ui/tokens] sync server listening on http://localhost:${port}`,
  )
  // eslint-disable-next-line no-console
  console.log(`[@primitiv-ui/tokens] writing JSON files to ${here}`)
})
