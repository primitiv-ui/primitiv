...
- `wrap` — whether long lines reflow (default `true`); see below.
...
## Wrapping — and when to turn it off

Long lines **wrap to the container** by default: on a narrow viewport the code
reflows instead of pushing a horizontal scrollbar, and `overflow-wrap: anywhere`
floors the min-content width at one glyph so a single unbreakable token cannot
hold the block open.

`wrap={false}` opts out, for content whose **column alignment is the content**:

```tsx
<CodeBlock wrap={false} language="tsx" code={anatomyTree} />
```

The case that drove it is the docs site's anatomy tree — component parts on the
left, the DOM each emits in an aligned trailing `//` comment. Wrapped, every
annotation dropped onto the next line and read as belonging to the part below.
With `wrap={false}` the lines keep their own width and the block scrolls inside
its own box, the same contract a wide table gets.

Reach for it only when reflowing actively changes the meaning; for ordinary
snippets the default is right, and the reasoning above is why. Note it applies to
the chip too, where a long line will then overflow its container — the `inline`
variant's own note explains why that default exists, so pass `wrap={false}`
there only deliberately.

## Files

| File | Authored? | Role |
|---|---|---|
| `contract.json` | **authored** | The styling contract — the `.primitiv-code-block` root class, its parts, the `variant`/`size`/`wrap` modifiers, and the `--primitiv-code-block-*` / `--primitiv-code-syntax-*` tokens. |
...
| `code-block.recipe.ts` | **authored** | A `cva("primitiv-code-block", ...)` with the `variant`, `size` and `wrap` axes. |
