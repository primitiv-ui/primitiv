/**
 * arrange-input-component-set.js
 *
 * Positions all variants in the Input component set into the documented grid.
 * Run this after building or restructuring variants.
 *
 * Grid structure:
 *   Rows    → Size (md first, then xs sm lg xl)
 *   Columns → State group (default / hover / focus / disabled / invalid)
 *               × Filled (empty / filled)
 *
 * Input has no Variant (intent) axis — it is a single visual style. The major
 * column axis is therefore State, with Filled as the sub-column (empty shows the
 * muted placeholder, filled shows the primary-coloured value).
 *
 * Density is not a grid dimension — it is controlled by the containing frame's
 * Context variable mode override in the design. The set shows variants at the
 * canvas default mode (Compact).
 *
 ...
 *
 * ─── Usage ───────────────────────────────────────────────────────────────────
 *  1. Select the Input component set on the canvas.
...
(async function () {
  ...
  const STATE_ORDER  = ["default", "hover", "focus", "disabled", "invalid"];
  const FILLED_ORDER = ["empty", "filled"];
  ...
  if (!componentSet) {
    console.error("Nothing selected. Select the Input component set and re-run.");
    return;
  }
  ...
  const valid = all.filter(c => {
    ...
    return p[PROP.size] && p[PROP.state] && p[PROP.filled];
  });
  ...
  // Default instance: md / default / empty
  ...
  // Labels
  const existing = figma.currentPage.findOne(n => n.name === "Input Grid Labels");
  ...
  for (const state of STATE_ORDER) {
    ...
    for (const filled of FILLED_ORDER) {
      ...
      const lbl  = makeLabel(filled, 0, cy - ABOVE_FILLED, false);
  ...
  labelGroup.name  = "Input Grid Labels";
