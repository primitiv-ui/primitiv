/**
 * arrange-segmented-control-item-component-set.js
 *
 * Positions all variants in the "Segmented Control / Item" component set into
 * the documented grid.
 *
 * Grid structure:
 *   Rows    → Size (md first, then xs sm lg xl)
 *   Columns → Selected group (true / false)
 ...
 *
 * ─── Usage ───────────────────────────────────────────────────────────────────
 *  1. Select the "Segmented Control / Item" component set on the canvas.
 ...
 *  (Or run via figma_execute, replacing the selection lookup with a
 *   getNodeByIdAsync call for the set.)
 * ─────────────────────────────────────────────────────────────────────────────
 */
(async function () {
  ...
  const PROP = { size: "Size", state: "Selected", interaction: "Interaction" };

  const SIZE_ORDER        = ["md", "xs", "sm", "lg", "xl"];
  const STATE_ORDER       = ["true", "false"];
  ...
  if (!componentSet) {
    console.error("Nothing selected. Select the Segmented Control / Item component set and re-run.");
    return;
  }
  ...
  // Default instance: md / true / default (the brand-filled "selected" hero cell)
  const defaultComp = valid.find(c => {
    const p = c.variantProperties;
    return p[PROP.size] === SIZE_ORDER[0] && p[PROP.state] === STATE_ORDER[0] && p[PROP.interaction] === INTERACTION_ORDER[0];
  });
  if (defaultComp) componentSet.insertChild(0, defaultComp);

  // Labels
  const existing = figma.currentPage.findOne(n => n.name === "Segmented Control / Item Grid Labels");
  ...
  for (const state of STATE_ORDER) {
    ...
    const sl = makeLabel(`Selected=${state}`, 0, cy - ABOVE_STATES, true);
  ...
  labelGroup.name  = "Segmented Control / Item Grid Labels";
