...
//
// Shown in STYLED mode (the mode switch reads "Styled"): the component has a
// visible styled surface, so the page leads with a Radix-style **Playground**
// (Variant / Size / Density controls) above the docs. Density is Primitiv's
// Context system (a <Density> ancestor / data-density), not a Button prop — a
// system concept demoed live. In Headless mode the playground degrades to a
// plain preview and variant/size drop from the API.
//
// POC of the generated docs-data pipeline (docs/docs-site-planning.md §1.5–1.8,
// §1.19): the props table (types/defaults/descriptions), the `extends` note and
// the CSS-variable list are laid out FROM generated data, not hand-typed. The
// `D` object below is a snapshot of scripts/docs-data/button.docs.json, produced
// by  node scripts/docs-data/extract-docs-data.mjs button  (TS compiler API over
// packages/react for the headless half — §1.14 propFilter drops native DOM
// attrs — merged with registry/components/button/contract.json for the styled
// half). A real docs build imports the JSON directly; the console can't read
// repo files, so a snapshot is embedded here. Numbered markers ①–⑩ map to notes.
...
return (async function () {
  ...
  const C = {
    white: "#FFFFFF", border: "#E0E0E0", dark: "#1E1E1E", muted: "#AAAAAA", sec: "#555555",
    ph: "#F8F8F8", sbBg: "#FBFBFC", active: "#ECECEF", codeBg: "#F5F5F7",
    note: "#FFF3CD", noteBorder: "#E8C766", noteText: "#7A5C00",
    styledTint: "#F1F6FF", styledTag: "#3B6FD4", styledBorder: "#CFE0FA",
  };
  ...
  const D = {
    ...
    importPath: "@primitiv-ui/react", extends: "HTMLButtonElement", rootClass: "primitiv-button",
    headless: [
  ...
  // desktop playground control: label above a value box
  function controlField(F, x, y, w, label, value) { text(F, label, x, y, 10, HEADM, C.muted, { spacing: 4 }); rect(F, x, y + 14, w, 36, C.white, { radius: 8, stroke: C.border }); text(F, value, x + 12, y + 24, 13, HEADM, C.dark); text(F, "▾", x + w - 26, y + 25, 13, HEADM, C.muted, { width: 14, align: "CENTER" }); }
  // mobile control / mode field: label left, value + chevron right
  ...
  // ============================================================ DESKTOP
  {
    ...
    let FH = 1900;
    text(page, "Docs Site — Component page (Button) · desktop · Styled mode · playground + props LAID OUT FROM generated docs-data", 0, -64, 22, HEAD, C.dark);
    ...
    const seg = (220 - 4) / 3;
    rect(F, modeX + 2 + seg, 16, seg, 32, C.dark, { radius: 6 });
    text(F, "Headless", modeX + 2, 24, 13, HEADM, C.sec, { width: seg, align: "CENTER" });
    ...
    // content
    let y = NAV + 40;
    text(F, "Docs", MAIN_X, y, 13, BODY, C.muted); text(F, "/", MAIN_X + 40, y, 13, BODY, C.muted);
    text(F, "Components", MAIN_X + 54, y, 13, BODY, C.muted); text(F, "/", MAIN_X + 138, y, 13, BODY, C.muted);
    ...
    // Playground (demo-first, styled)
    heading("Playground", 5);
    const pgH = 250;
    rect(F, MAIN_X, y, MAIN_W, pgH, C.white, { radius: 12, stroke: C.border });
    text(F, "PREVIEW", MAIN_X + 16, y + 14, 10, HEADM, C.muted, { spacing: 4 });
    rect(F, MAIN_X + MAIN_W / 2 - 55, y + 66, 110, 42, C.dark, { radius: 8 });
    text(F, "Button", MAIN_X + MAIN_W / 2 - 55, y + 78, 14, HEAD, C.white, { width: 110, align: "CENTER" });
    rect(F, MAIN_X, y + 170, MAIN_W, 1, C.border);
    const fw = (MAIN_W - 32 - 32) / 3;
    controlField(F, MAIN_X + 16, y + 186, fw, "VARIANT", "primary");
    controlField(F, MAIN_X + 16 + fw + 16, y + 186, fw, "SIZE", "md");
    controlField(F, MAIN_X + 16 + 2 * (fw + 16), y + 186, fw, "DENSITY", "Comfortable");
    y += pgH + 12;
    text(F, '<Button variant="primary" size="md">Button</Button>', MAIN_X, y, 13, BODY, C.sec, { width: MAIN_W });
    y += 22;
    text(F, "Density is set by a <Density> ancestor (data-density) — the Context system, not a Button prop.", MAIN_X, y, 12, BODY, C.muted, { width: MAIN_W });
    y += 40;

    // Installation (styled)
    heading("Installation", 6);
    pmCodeBlock(F, MAIN_X, y, 470, "npx " + D.styledInstall);
    text(F, 'import { Button } from "@/components/ui/button"', MAIN_X + 490, y + 8, 13, BODY, C.sec, { width: 250, lineHeight: 18 });
    text(F, "Copied into your project · ." + D.rootClass, MAIN_X + 490, y + 46, 13, BODYM, C.dark);
    text(F, "Headless mode → npm i " + D.importPath, MAIN_X + 490, y + 66, 12, BODY, C.muted, { width: 250, lineHeight: 16 });
    y += 108;

    // Props
    heading("Props", 7);
    text(F, "Extends " + D.extends + "  ·  headless props + styling-contract props", MAIN_X, y - 8, 13, BODY, C.muted);
    y += 20;
    [["Prop", 0], ["Type", 150], ["Default", 440], ["Description", 540]].forEach(c => text(F, c[0], MAIN_X + c[1], y, 12, HEADM, C.muted, { spacing: 3 }));
    y += 22;
    rect(F, MAIN_X, y, MAIN_W, 1, C.border); y += 8;
    const propRow = (p, con) => {
      const rh = 50;
      if (con) rect(F, MAIN_X - 8, y - 2, MAIN_W + 16, rh, C.styledTint, { radius: 6 });
      text(F, p.n, MAIN_X, y + 6, 14, HEADM, C.dark);
      if (con) { rect(F, MAIN_X + p.n.length * 8 + 12, y + 7, 58, 16, C.white, { radius: 8, stroke: C.styledBorder }); text(F, "contract", MAIN_X + p.n.length * 8 + 12, y + 8, 9, HEADM, C.styledTag, { width: 58, align: "CENTER" }); }
    ...
    y += 12;
    text(F, "Tinted rows are added by the styling contract — absent in Headless mode.", MAIN_X, y, 12, BODY, C.muted);
    y += 34;

    // Styling contract
    heading("Styling contract", 8);
    ...
    // Accessibility
    heading("Accessibility", 9);
    ["Space / Enter activates the button.", "Visible focus ring via :focus-visible.", "disabled exposes data-disabled + native disabled semantics.", "asChild keeps a11y wiring when rendering a link."].forEach(b => { rect(F, MAIN_X, y + 7, 5, 5, C.dark, { radius: 3 }); text(F, b, MAIN_X + 16, y, 14, BODY, C.sec, { width: MAIN_W - 16 }); y += 28; });
    y += 24;
    ...
    marker(F, 10, TOC_X + 132, ty - 4, 18);
    ty += 26;
    ["Playground", "Installation", "Props", "Styling contract", "Accessibility", "Examples"].forEach((t, i) => { if (i === 0) rect(F, TOC_X - 12, ty - 2, 3, 18, C.dark); text(F, t, TOC_X, ty, 14, i === 0 ? HEADM : BODY, i === 0 ? C.dark : C.muted); ty += 28; });

    FH = Math.max(contentBottom, ty) + 48;
    F.resize(W, FH); sbBg.resize(SB_W, FH - NAV); sbBorder.resize(1, FH - NAV);
  }

  // ============================================================ MOBILE
  {
    const W = 390, M = 20, CW = W - M * 2, X0 = 1560;
    text(page, "Component page — Button (mobile · Styled)", X0, -64, 22, HEAD, C.dark);
    const F = figma.createFrame();
    F.name = "Component page — Button (mobile)"; F.x = X0; F.y = 0; F.resize(W, 2600);
    F.fills = solid(C.white); F.clipsContent = true; page.appendChild(F);
    ...
    let y = 128;
    text(F, "Components  /  ", M, y, 12, BODY, C.muted); text(F, "Button", M + 88, y, 12, BODYM, C.dark);
    ...
    // Playground
    heading("Playground", 5);
    rect(F, M, y, CW, 120, C.white, { radius: 12, stroke: C.border });
    text(F, "PREVIEW", M + 14, y + 12, 10, HEADM, C.muted, { spacing: 4 });
    rect(F, M + CW / 2 - 48, y + 50, 96, 40, C.dark, { radius: 8 });
    text(F, "Button", M + CW / 2 - 48, y + 61, 14, HEAD, C.white, { width: 96, align: "CENTER" });
    y += 130;
    dropdownField(F, M, y, CW, 42, "Variant", "primary"); y += 50;
    dropdownField(F, M, y, CW, 42, "Size", "md"); y += 50;
    dropdownField(F, M, y, CW, 42, "Density", "Comfortable"); y += 58;
    text(F, "Density = the Context system (a <Density> ancestor), not a Button prop.", M, y, 12, BODY, C.muted, { width: CW, lineHeight: 16 });
    y += 40;

    // Installation
    heading("Installation", 6);
    pmCodeBlock(F, M, y, CW, "npx " + D.styledInstall, { copy: false });
    y += 96;
    text(F, 'import { Button } from "@/components/ui/button"', M, y, 12, BODY, C.sec, { width: CW });
    y += 22;
    text(F, "Headless mode → npm i " + D.importPath, M, y, 12, BODY, C.muted, { width: CW });
    y += 40;

    // Props (cards)
    heading("Props", 7);
    text(F, "Extends " + D.extends + "  ·  headless + contract props", M, y - 6, 12, BODY, C.muted, { width: CW });
    y += 20;
    const propCard = (p, con) => {
      const ch = 104;
      rect(F, M, y, CW, ch, con ? C.styledTint : C.white, { radius: 10, stroke: con ? C.styledBorder : C.border });
      text(F, p.n, M + 14, y + 12, 16, HEADM, C.dark);
      if (con) { rect(F, M + 14 + p.n.length * 9 + 8, y + 13, 58, 16, C.white, { radius: 8, stroke: C.styledBorder }); text(F, "contract", M + 14 + p.n.length * 9 + 8, y + 14, 9, HEADM, C.styledTag, { width: 58, align: "CENTER" }); }
    ...
    y += 2;
    text(F, "Tinted = added by the styling contract (absent in Headless).", M, y, 12, BODY, C.muted, { width: CW });
    y += 34;
    ...
    // Accessibility
    heading("Accessibility", 9);
    ["Space / Enter activates the button.", "Visible focus ring via :focus-visible.", "disabled exposes data-disabled semantics.", "asChild keeps a11y wiring for a link."].forEach(b => { rect(F, M, y + 7, 5, 5, C.dark, { radius: 3 }); text(F, b, M + 14, y, 14, BODY, C.sec, { width: CW - 14, lineHeight: 20 }); y += 30; });
    y += 20;
  ...
  // ============================================================ NOTES
  {
    ...
    text(NP, "Shown in STYLED mode. Props/types/defaults/extends + CSS vars laid out FROM generated docs-data (scripts/docs-data/button.docs.json). Not hand-typed.", 28, 58, 12.5, BODY, C.noteText, { width: 445, lineHeight: 17 });
    const notes = [
      ["1", "Persistent docs shell — same top nav as the landing (audience fork, framework + mode switches, search, theme)."],
      ["2", "Left sidebar = the §1.4 nav tree; Components expanded, Button active. Moves between component pages."],
      ["3", "Breadcrumb Docs / Components / Button — the connection back up. Reached from landing via 'Browse Components' → Components → Button."],
      ["4", "Mode switch changes page CONTENT. Styled shown: playground + contract props + 'primitiv add'. Headless: playground degrades to a plain preview, variant/size drop, install → npm. Figma → spec/redline (§1.1)."],
      ["5", "Playground (demo-first, Radix-style) — Variant / Size / Density controls. Density = Primitiv's Context system (a <Density> ancestor), not a prop — a system concept a generic library can't demo. Styled-surface feature."],
      ["6", "Install block — mode-aware + package-manager tabbed. Styled: npx primitiv add button (copied into project); Headless: npm i @primitiv-ui/react."],
      ["7", "Props table GENERATED — 4 headless props (extractor, native DOM attrs filtered) + variant/size from contract.json. Tinted 'contract' rows are styled-only, absent in Headless (§1.7)."],
      ["8", "Styling contract = the 14 --primitiv-button-* CSS vars; mode-agnostic, shown regardless of the switch (§1.5/§1.6)."],
      ["9", "Accessibility — hand-authored (not extractable; §1.7 a11y)."],
      ["10", "On-this-page TOC (desktop right rail). Mobile: sidebar → hamburger, TOC dropped, props → stacked cards, playground controls stacked."],
    ];
    let ny = 112;
    notes.forEach(n => { const c = figma.createEllipse(); c.x = 28; c.y = ny; c.resize(20, 20); c.fills = solid(C.dark); NP.appendChild(c); text(NP, n[0], 28, ny + 3, 11, HEADM, C.white, { width: 20, align: "CENTER" }); const t = text(NP, n[1], 60, ny - 1, 13, BODY, C.dark, { width: 410, lineHeight: 18 }); ny += Math.max(52, t.height + 14); });
    NP.resize(500, ny + 12);
  }
